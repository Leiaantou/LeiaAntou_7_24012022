<template>
  <div class="bloc-modale" v-if="reveleComment">
    ...
    <div class="modale card">
      <div v-on:click="toggleModaleComment" class="btn-modale btn btn-danger">X</div>
      ...
      <div class="form-group">
          ...
          <textarea rows="5" class="form-control" v-model="commentModified.content"></textarea>
      </div>
      <div class="d-flex flex-row align-items-center mb-3">
      <p class="mr-5">Image d'origine :</p>
      <img class="preview" v-if="comment.image != null" :src="
      ...
      <button class="btn btn-info" @click="editComment(comment.id)" v-on:click="toggleModaleComment"><fa icon="paper-plane" /></button>
      </div>
      </div>
  </div>
</template>
    ...
    getAllPosts() {
      axios
        ...
        onFileSelected(event){
                 console.log(event);
          this.commentModified.image = event.target.files[0] || event.dataTransfer.files;
        ...
        editComment(commentId){
          ...
          axios
              .put("http://localhost:3000/api/comment/" + commentId, formData , { headers: {
